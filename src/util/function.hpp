...
template <int N, int M, typename T, int K, int L>
void Function::conv2d(Tensor<N, M, T>& t, Tensor<K, M+1, T>& w, Tensor<L, M, T> *ans, int strides) {
  ans->init();
...
template <int N, int M, typename T, int L>
void Function::max_pool(Tensor<N, M, T>& t, int k_col, int k_row, Tensor<L, M, T>* ans, int strides) {
  int* ans_dim = ans->shape();
  int* dim = t.shape();
  for (int k = 0; k < ans_dim[2]; ++k)
    for (int i = 0; i < ans_dim[1]; ++i)
